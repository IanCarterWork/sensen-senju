...
export function Compilate(config, input, index){
    ...
    const isImport = input.match( 
        
        new RegExp(`(${ 
            
            path.basename( config.front.paths.app ) 
        
        })(.*)\.(html|htm|sense|activity|component|modal|css|s[ac]ss|less)$`, 'g') 
...
export function Recoltes(config){
    ...
    return ( 
        ...
        glob.readdirSync(`${ path.relative(process.cwd(), config.front.paths.app ) }/**/*`) || [] 
