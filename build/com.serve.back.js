...
export async function ServeBack() {
    ...
    if (configCombos.back) {
        const root = Project.Path(`${configCombos.project.path.back}/${configCombos.back.paths.public}`);
        ...
        Project.BackendCommand('Backend Server', `cd "${root}" && php -S ${host}:${port}`, configCombos);
